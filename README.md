...
- 🌐 [Custom Domain + DNS Record](#custom-domain-+-dns-record) - Creates a custom domain with ACM via domain_names; optional Route53 alias via dns_records
...
Use `domain_names` (FQDN keys) to create `aws_api_gateway_domain_name` / `aws_api_gateway_base_path_mapping` with `certificate_arn` (and optional `base_path` / `stage_name`). Optionally set `dns_records` to create a Route53 A-alias; the derived FQDN (`<record_name>.<zone_name>`, or bare zone for `"_null_"`) must match a key in `domain_names`. Omit `dns_records` or set `dns_records = {}` for custom domains with external DNS.
...
| domain_names                 | Map of custom domain names (FQDN keys) with `certificate_arn` and optional `base_path` / `stage_name`.                                                                          | `map`    | `{}`                                                     | no       |
| dns_records                  | Optional Route53 A-aliases; key is the record name (`""` uses `${local.common_name}-${each.key}`, `"_null_"` uses the zone apex). Derived FQDN must match a `domain_names` key. | `map`    | `{}`                                                     | no       |
...
- **⚠️ DNS Records:** `dns_records` creates Route53 A-aliases only; the FQDN must match a key in `domain_names`. Ensure the hosted zone exists. For external DNS, configure `domain_names` and omit `dns_records` (or set `{}`).
